...
<script type="text/javascript" language="javascript">
	...
	function collect(value,topicid){
		var msg = '<s:property value="#session.tu.username"/>';
		if(!msg){
			var returnVal = window.confirm("未登录或登录已失效！请登录！", "提示");
			if (returnVal) {
			...
			if(value=='收藏该贴'){
				...
				window.location.href="favor_addFavorByTopicidAndUserid.action?topic.id="+topicid
						+"&&users.id="+'<s:property value="#session.tu.id"/>'
				
			}else{
				$("#collect").val("收藏该贴");
				window.location.href="favor_deleteFavorByTopicidAndUserid.action?topic.id="+topicid
				+"&&users.id="+'<s:property value="#session.tu.id"/>'
			...
			<div class="titleStyle" align="left">
				<div style="width:800px;float: left;font-size: 13px; ">
					...
					<font style="font-size: 13px;color: white">[积分: <s:property
							value="topic.integral" /> 分]
					</font>
					<c:if test="${isCollect==1 }">
						<input id="collect" class="collect" type="button" value="已收藏"
				...
				<div class="userInfo">
					<div style="height:180px;" align="center">
						<img width="120px;" height="160px;" style="padding-top: 20px;"
							onerror="this.src='./image/ds-java.png'"
