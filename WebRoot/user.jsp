...
function focusperson(value,userid){
	//alert("你好");
	...
	if(!msg){
		...
		if(value=='关注'){
			$("#collect").val("取关");
			window.location.href="focus_FocusPerson.action?user.id="+userid;
			
		}
		//取关触发的事件
		else{
			$("#collect").val("关注");
			window.location.href="focus_UnFocusPerson.action?user.id="+userid;
		}
	} 
}
var focusmsg="${requestScope.focusmsg}";
if(focusmsg!=""){
	alert(focusmsg);
}
</script>
				...
				<div  align="left" style="width:400px;float: left;margin-left: 30px;">
					<table>
						...
						<tr>
							<td>
								<c:if test="${isfocus==0 }">
									<input id="collect" class="collect" type="button" value="关注"
										onclick="focusperson(this.value,'<s:property value="user.id"/>')" />
								</c:if>
								<c:if test="${isfocus==1 }">
									<input id="collect" class="collect" type="button" value="取关"
										onclick="focusperson(this.value,'<s:property value="user.id"/>')" />
<%-- 									<input id="collect" class="collect" type="button" value="取关"
										onclick="focusperson('取关','<s:property value="user.id"/>')" /> --%>
								</c:if>
							</td>
						</tr>
						<%-- </c:if> <c:if test="${isCollect==0 }">
								<input id="collect" class="collect" type="button" value="收藏该贴"
									onclick="collect(this.value,'<s:property value="topic.id"/>')" />
							</c:if> --%>
						<%-- <tr>
							<td><a href="focus_FocusPerson.action?user.id=<s:property value="user.id" />" style="color: blue" onclick="javascript:alert('成功关注')">点击关注</a></td>
							<td><a href="focus_UnFocusPerson.action?user.id=<s:property value="user.id" />" style="color: blue" onclick="javascript:alert('成功取关')">取消关注</a></td>
						</tr> --%>
					</table>
				</div>
			</div>
